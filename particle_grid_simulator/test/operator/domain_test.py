...
# ==========================================
# NEW OPERATOR DOMAIN INTERFACES (MOCKED)
# ==========================================
...
def field_evolution_rule(data_context: tuple) -> State:
    ...
    # Extract mass and normalize it to create a probability distribution
    ...
    # Select index based on field distribution
...
def plot_trajectory(history: list, title: str, filename: str):
    ...
    if len(X) > 1:
        u = np.diff(X)
        v = np.diff(Y)
        # Using quiver to draw exact arrows pointing from n to n+1
...
def run_field_test():
    ...
    field_op = GenericOperatorData(field_evolution_rule, State)
    current_state = State(np.array([0.0, 0.0], dtype=np.float64))
    history = [current_state.vector]
    ...
    # Build the entire universe geometry in memory before the clock starts.
    # ==========================================
    ...
    # Warmup the topology far enough that the particle can never walk off the edge
    topology_cm.warmup([current_state], steps=ITERATIONS + 2)

    # Extract the EXACT coordinates the topology just mapped
    universe_coords = np.array(topology_cm.fast_refs.handle_map)
    universe_weights = np.ones((len(universe_coords), 1), dtype=np.float64)

    # Wrap the physical universe in the 1.0 mass field
    ...
    # Wire all the memory pointers together EXACTLY ONCE
    ...
    # ==========================================
    # 2. THE PURE HOT LOOP
    # Zero memory allocation, zero topology expansion. Just pure math.
    # ==========================================
    for i in range(ITERATIONS):
        ...
        current_state = GenericOperatorUtility.evolve(field_op, (final_states[mask], final_fields[mask]))
        history.append(current_state.vector)
    ...
    plot_trajectory(history, f"Field Random Walker ({ITERATIONS} Iterations)", "Domian_field_walker_trajectory.png")
